...
int main()
{
	...
	fout.close();
	system("rm ./svm/*");
	system("./libsvm/svm-train -h 0 train.csv");
	system("./libsvm/svm-predict predict.csv train.csv.model ./svm/svm");
	string name[100];
	for(int i = 0; i < time; i++)
	{
		cout<<"Time: "<<i<<endl;
		string prec = "./libsvm/svm-predict predict.csv train.csv.model ";
		stringstream ss;
		ss << "./svm/svm" << i;
		name[i] = ss.str();
		prec += name[i];
		...
		system(prec.c_str());
		system("rm train.csv");
		system("rm train.csv.model");
	}
	system("rm predict.csv");
